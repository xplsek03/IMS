...
struct ar {
	...
	int zasoba_stutoxu; // pocet granuli stutoxu za obdobi. max 128 granuli na ctverec pri 2hg/ha
			...
			for(int j = 0; j < 100; j++) {
				...
				if(c == 16) {				
					...
					if(pole[i][j].zasoba_stutoxu > 0) { // jeste muzeme nasypat jed
						int mn = range(2,4) * buffer[i][j]; // 2 - 4 granule/noru * pocet nor
						...
						for(int a = 0; a < stav; a++) {
							...
							if(((rand() % 100) < 85) && (ubytek < stav) && (pole[i][j].stutox > 0)) { // ucinnost jedu 95%
								ubytek++;
								pole[i][j].stutox--;
							}
						}
