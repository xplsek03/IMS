				...
				if(c >= 16 && c <= 21 && !pole[i][j].plodina) {				
					...
					if(pole[i][j].zasoba_stutoxu > 0) { // jeste muzeme nasypat jed
						...
						for(int a = 0; a < stav; a++) {
							...
							if(((rand() % 100) < 65) && (ubytek < stav) && (pole[i][j].stutox > 0)) { // ucinnost jedu 95% o neco snizena velkoplosnou aplikaci, navic snizeno opakovanou navnadou
								ubytek++;
								pole[i][j].stutox--;
							}
						}
